...
static void test_program() {
    ...
    ape_set_external_function(ape, "external_fn_test", external_fn_test, &g_external_fn_test);
    ape_set_global_constant(ape, "test", ape_object_make_number(42));
    ape_set_external_function(ape, "square_array", square_array_fun, NULL);
    ape_set_external_function(ape, "make_test_dict", make_test_dict_fun, NULL);
    ape_set_external_function(ape, "test_check_args", test_check_args_fun, NULL);
...
static void test_calling_functions() {
    ...
    ape_set_external_function(ape, "add", add_fun, NULL);
    ape_set_external_function(ape, "fourtytwo", fourtytwo_fun, NULL);
    ...
    ape_object_t res = ape_execute(ape, test_stack_corruption_code);
...
static void test_traceback() {
    ...
    ape_set_external_function(ape, "custom_error", custom_error_fun, NULL);
...

    {
        ...
        for (int i = 0; i < APE_ARRAY_LEN(tests); i++) {
            ...
            assert(ape_traceback_get_line_number(traceback, i) == test.line);
            assert(ape_traceback_get_column_number(traceback, i) == test.column);
            assert(APE_STREQ(ape_traceback_get_function_name(traceback, i), test.name));
        }
    }
...
static ape_object_t make_test_dict_fun(ape_t *ape, void *data, int argc, ape_object_t *args) {
    ...
    for (int i = 0; i < num_items; i++) {
        ...
        ape_object_set_map_value(res, key, val);
    }
    return res;
}
